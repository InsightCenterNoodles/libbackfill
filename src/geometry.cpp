...
void compress(std::span<const Vertex>    src,
    ...
    for (size_t i = 0; i < vertex_count; ++i) {
        ...
        out[i].tangent = packSnorm16(q);
    }
}

static SharedArray<PackedVertex> make_sphere() {
    ...
    for (int i = 0; i < ret.size(); i++) {
        ...
        auto position = p;
        auto normal   = normalize(position);
    ...
    compress(ret, std::span<const ushort3>(SPHERE_INDEX), out);

    return SharedArray<PackedVertex>::from_copy(out);
}

SharedArray<PackedVertex> sphere_verts() {
    static SharedArray<PackedVertex> data = make_sphere();
    return data;
}
SharedArray<filament::math::ushort3> sphere_index() {
    auto const* data = std::data(SPHERE_INDEX);
    size_t      sz   = std::size(SPHERE_INDEX);
    return SharedArray<filament::math::ushort3>::from_copy(
        std::span<const ushort3>(data, sz));
}

void LocalVertexBuffer::completion(void* buffer, size_t size, void* user) {
    auto* ptr = (LocalVertexBuffer*)user;

    ptr->m_pending_upload = SharedArray<PackedVertex>();
}

LocalVertexBuffer::LocalVertexBuffer(filament::Engine*         engine,
                                     SharedArray<PackedVertex> buffer) {
...
void LocalIndexBuffer::completion(void* buffer, size_t size, void* user) {
    ...
    ptr->m_pending_upload = SharedArray<filament::math::ushort3>();
}

LocalIndexBuffer::LocalIndexBuffer(
    filament::Engine*                    engine,
    SharedArray<filament::math::ushort3> buffer) {

    m_index_count = buffer.size() * 3;

    m_pending_upload = buffer;

    auto* ib = filament::IndexBuffer::Builder()
                   .indexCount(m_index_count)
